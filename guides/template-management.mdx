...
- Automated workflows that run after cluster creation
...
<Tabs>
  ...
  <Tab title="With All Options">
    ...
    hyve template create prod-template \
      ...
      --ingress \
      --load-balancer \
      --workflows setup-monitoring,deploy-app,configure-backups
    ```
  </Tab>
...
spec:
  ...
  ingress:                         # Optional: Ingress configuration
    enabled: true
    loadBalancer: true
  workflows:                       # Optional: Workflows to run after creation
    - workflow-name-1
    - workflow-name-2
```
...
4. Syncs kubeconfig
5. Executes all defined workflows in order
...
<Tabs>
  ...
  <Tab title="With Warnings">
    ...
    ⚠️  Warnings:
      ...
      • Workflow 'deploy-app' not found in repository
...
<Steps>
  <Step title="Create template once">
    ...
    hyve template create feature-env \
      ...
      --workflows deploy-app,seed-data
    ```
  </Step>
...
hyve template create customer-template \
  ...
  --workflows setup-tenant,deploy-app,configure-isolation
...
<CardGroup cols={2}>
  ...
  <Card title="CLI: Template Commands" icon="terminal" href="/cli/template">
