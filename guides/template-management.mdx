...
Templates combine:
- Cluster specifications (provider, region, nodes)
...
spec:
  ...
  nodes:                           # Required: List of node sizes
    - g4s.kube.large
    - g4s.kube.large
...
EKS cluster using named aliases from your AWS provider config:
...
metadata:
  name: aws-eks-template
  description: Standard AWS EKS cluster
spec:
  provider: aws
  region: us-east-1
  nodes:
