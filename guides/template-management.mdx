...
```yaml
spec:
  provider: civo                   # Required: Cloud provider
  region: NYC1                     # Required: Deployment region
  nodes:                           # Required: List of node sizes
    - g4s.kube.large
    - g4s.kube.large
  clusterType: k3s                 # Optional: k3s or talos (default: k3s)
...
2. Provisions cluster in cloud provider
3. Waits for cluster to be ready
4. Syncs kubeconfig
5. Executes all `onCreated` workflows in order
6. Note: `onDestroy` workflows run when the cluster is deleted
