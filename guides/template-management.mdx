...
spec:
  ...
  schedule: "0 20 * * 5"          # Optional: cron expression — auto-expire each cluster
                                   ...
                                   #   is computed at execute time and written to
                                   #   spec.expiresAt on the generated cluster.

  # AWS-specific fields (only when provider: aws)
  awsAccount: main-account         # Account alias from provider-configs/aws/main-account.yaml
...
2. **For AWS**: resolves `awsVpcName`, `awsEksRole`, and `awsNodeRole` aliases to VPC IDs and role ARNs from `provider-configs/aws/<account>.yaml`
  ...
  nodeGroups:
    ...
    - name: workers
      ...
      labels:
        role: worker
  clusterType: eks
  awsAccount: main-account    # matches the filename in provider-configs/aws/main-account.yaml
  awsVpcName: k8svpc          # matches vpcs[*].name in that account file
  awsEksRole: k8srole         # matches roles[*].name (EKS cluster role)
  awsNodeRole: k8snoderole    # matches roles[*].name (EKS node role)
  ingress:
    enabled: false
    loadBalancer: false
  workflows:
...
<Info>
  AWS alias names (`awsAccount`, `awsVpcName`, `awsEksRole`, `awsNodeRole`) are resolved to actual IDs and ARNs from `provider-configs/aws/<account>.yaml` when `hyve template execute` runs. See the [AWS provider config](/configuration) for how to define these aliases.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="AWS Alias Not Found">
    ...
    **Cause:** The alias name in the template doesn't match any entry in `provider-configs/aws/<account>.yaml`.

    **Solution:** Verify your provider config contains matching alias names:
    ```yaml
    # provider-configs/aws/main-account.yaml
