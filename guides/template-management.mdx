...
<Tabs>
  ...
  <Tab title="Manual YAML (AWS)">
    ...
    spec:
      ...
      awsVpcId: vpc-0abc123456789 # VPC ID (set in template or override at execute time)
...
spec:
  ...
  awsVpcId: vpc-0abc123456789      # VPC ID (set here or override at execute time)
...
2. **For AWS**: constructs role ARNs from `awsEksRoleName`/`awsNodeRoleName` and the account ID
...
<Info>
  `awsVpcId` and role names can be set directly in the template YAML or overridden at execute time via `--vpc-id`, `--eks-role-name`, and `--node-role-name` flags. Role ARNs are constructed automatically from the role name and account ID at reconcile time.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="AWS VPC or Role Not Set">
    **Problem:** `hyve template execute` fails because `awsVpcId` or `awsEksRoleName` is empty.

    **Solution:** Set the values in the template YAML directly or override at execute time:
    ```bash
    ...
    Alternatively, use `hyve interactive` to execute the template — the TUI offers live cloud lookup for VPCs and IAM roles.
  </Accordion>
