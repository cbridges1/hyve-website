...
| Auth (get kubeconfig) | Cluster selected from list; runs module auth operation to merge kubeconfig into `~/.kube/config`. |
...
| Action | Description |
|--------|-------------|
| Create | Driver source, driver version, region, and params collected via form. |
| Execute | Template selected from list; cluster name and optional param overrides collected. |
| List | Prints all templates. |
| Show | Template selected from list. |
| Validate | Template selected from list; checks driver is locked and workflows exist. |
