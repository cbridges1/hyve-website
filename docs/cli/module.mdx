...
Add a module at a specific version and write it to `hyve.lock`.

```bash
hyve module add <source>@<version>
```

<ParamField path="source@version" type="string" required>
  Module source and version joined with `@`. Examples:
...
```bash
# Add the first-party Civo module
...
hyve module add ./my-local-module@local
```
...
`hyve.lock` is a JSON file at the repository root:
...
```json
{
  "modules": {
    ...
    "github.com/hyve-modules/eks@v2.1.3": {
      ...
      "url": "https://github.com/hyve-modules/eks/archive/refs/tags/v2.1.3.tar.gz"
    }
  }
}
```
