...
<Info>
  Auth is implemented by the module — specifically the `auth.yaml` file in the module directory. Different modules use different mechanisms: `civo kubernetes config --merge`, `aws eks update-kubeconfig`, `gcloud container clusters get-credentials`, or `az aks get-credentials`.
</Info>
