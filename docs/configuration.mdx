...
<Warning>
  Enable `strictDelete` only after ensuring your state repository contains definitions for every cluster you want to keep. Any cloud cluster without a matching YAML file will be permanently deleted during reconciliation.
</Warning>
...
`hyve.lock` is a content-addressed lock file at the root of your state repository. It is auto-managed by the `hyve module` commands and should be committed to Git.
...
```json
{
  "modules": {
    "github.com/hyve-modules/civo@v1.0.0": {
      ...
      "url": "https://github.com/hyve-modules/civo/archive/v1.0.0.tar.gz"
    }
  }
}
```

You should never edit `hyve.lock` directly. Use the module commands to manage it:

```bash
hyve module add github.com/hyve-modules/civo@v1.0.0    # add and lock
hyve module update github.com/hyve-modules/civo         # update to latest
...
└── repositories/       # Cloned repository storage
    └── <repo-name>/
        ...
        └── hyve.lock       # Module lock file (auto-managed)
```
