...
The `auth.yaml` file has `kind: ClusterAuth`. Hyve runs this operation when `hyve cluster auth <name>` is called. The operation is expected to merge the cluster's kubeconfig into `~/.kube/config` using standard Kubernetes tooling (e.g. `kubectl config merge`, `KUBECONFIG` env var merge, or the cloud CLI's built-in merge command).

```bash
# Example auth.yaml step for Civo
civo kubernetes config ${HYVE_CLUSTER_NAME} --merge --save
```

```bash
# Example auth.yaml step for EKS
...
```json
{
  "modules": {
    "github.com/hyve-modules/civo@v1.0.0": {
      ...
      "url": "https://github.com/hyve-modules/civo/archive/refs/tags/v1.0.0.tar.gz"
    }
  }
}
```
