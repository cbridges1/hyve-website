...
<ParamField path="spec.lockParams" type="boolean">
  When `true`, users cannot override default param values at execute time. The TUI skips the "Override default params?" prompt entirely, and any `--set` flags passed to `hyve template execute` are silently ignored. Use this to enforce a fixed cluster shape — for example, a production template where node size and count must not be changed by the person spinning up the cluster.
...
```bash
# Execute template
hyve template execute prod-template my-prod-cluster
...
```bash
# US East
hyve template execute prod-template us-east-prod
# Edit template to change region to NYC1

# US West
hyve template execute prod-template us-west-prod
# Edit template to change region to PHX1

# EU
hyve template execute prod-template eu-prod
...
hyve template execute prod-template test-new-config

# If successful, use in production
hyve git use production
hyve template execute prod-template prod-new-cluster
```
...
<AccordionGroup>
  ...
  <Accordion title="4. Version Templates">
    ...
    hyve template execute prod-template my-cluster
    ```
  </Accordion>
  ...
  <Accordion title="5. Test Templates">
    ...
    # Test in development
    hyve git use development
    hyve template execute new-template test-cluster

    # Verify workflows run successfully
    # Then use in production
    hyve git use production
    hyve template execute new-template prod-cluster
    ```
  </Accordion>
</AccordionGroup>
...
```bash
# Create production cluster
hyve template execute production-complete prod-app-01
...
<CardGroup cols={2}>
  ...
  <Card title="Template Management" icon="stamp" href="/docs/guides/template-management" color="#06b6d4">
    Complete template guide
  </Card>

  <Card title="CLI Template" icon="terminal" href="/docs/cli/template" color="#10b981">
