...
<ParamField path="spec.params" type="object">
  ...
  Params are set when creating a template (`--set KEY=VALUE`) or can be overridden at execute time.
</ParamField>
...
Or use a template with a `schedule` to compute the expiry automatically at execute time.
