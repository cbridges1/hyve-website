...
<ParamField path="spec.workflows.onCreate" type="string[]">
  Workflow names to run after the cluster is created and the module reports status `active`. The cluster exists; run `hyve cluster auth` first if your workflows need `kubectl` access.
</ParamField>
...
<Steps>
  ...
  <Step title="Reconcile">
    ...
    hyve reconcile
    ```
    Runs `beforeCreate` workflows, calls the module's create operation, waits for `active` status, runs `onCreate` workflows.
  </Step>
...
| `onCreate` | After cluster reaches `active` status | Yes | After auth |
