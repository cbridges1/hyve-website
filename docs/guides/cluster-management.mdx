...
```bash
hyve template execute prod-civo my-prod-cluster
```

**Override template defaults at execute time:**

```bash
# Override region and node count for this specific cluster
hyve template execute prod-civo eu-cluster \
  --region FRA1 \
  --set node_count=5
```
...
hyve template execute prod-civo staging-cluster --set node_count=2

# Switch to production
hyve git use production
hyve template execute prod-civo prod-cluster-01
```
...
hyve template execute prod-civo-large my-new-cluster
```
...
hyve template execute dev-weekly my-dev-cluster
```
...
hyve template execute prod-civo prod-green \
  --set node_size=g4s.kube.large \
  --set node_count=3
