...
<ParamField path="spec.workflows.onCreated" type="array">
  List of workflow names to execute after cluster creation
</ParamField>

<ParamField path="spec.workflows.onDestroy" type="array">
  List of workflow names to execute before cluster destruction
</ParamField>
...
hyve template create prod-template \
  --region NYC1 \
  --nodes g4s.kube.large,g4s.kube.large,g4s.kube.large \
  --on-created setup-monitoring,deploy-apps \
...
# 3. Run 'onCreated' workflows (setup-monitoring, deploy-base-apps)
# Note: 'onDestroy' workflows run when the cluster is deleted
```
...
<Steps>
  ...
  <Step title="Execute onCreated Workflows">
    Run each onCreated workflow in sequence

    ```
    [INFO] Running onCreated workflow 'setup-monitoring'
    [INFO] Workflow 'setup-monitoring' completed
    [INFO] Running onCreated workflow 'deploy-base-apps'
...
|-------|--------------|-----------|
| `onCreated` | After cluster is ready | Setup monitoring, deploy apps, configure ingress |
| `onDestroy` | Before cluster deletion | Backup data, cleanup DNS, notify teams |
...
# [INFO] Running onCreated workflow 'setup-monitoring'
# [INFO] Workflow 'setup-monitoring' completed
# [INFO] Running onCreated workflow 'setup-logging'
...
# Output:
# [INFO] Running onDestroy workflow 'backup-persistent-volumes'
# [INFO] Workflow 'backup-persistent-volumes' completed
# [INFO] Running onDestroy workflow 'export-audit-logs'
# [INFO] Workflow 'export-audit-logs' completed
# [INFO] Running onDestroy workflow 'notify-operations-team'
