...
Templates are YAML files that define cluster specifications and optional workflows:
...
<ParamField path="spec.workflows" type="array">
...
hyve template create prod-template \
  --region NYC1 \
  --nodes g4s.kube.large,g4s.kube.large,g4s.kube.large \
  --workflows setup-monitoring,deploy-apps
```
...
# 3. Run 'setup-monitoring' workflow
# 4. Run 'deploy-base-apps' workflow
```
...
<Steps>
  ...
  <Step title="Execute Workflows">
    ...
    ```
    [INFO] Running workflow 'setup-monitoring'
    [INFO] Workflow 'setup-monitoring' completed
    [INFO] Running workflow 'deploy-base-apps'
...
Templates can automatically run workflows after cluster creation:

```yaml
# Template with workflows
spec:
  cluster:
    region: NYC1
    nodes:
      ...
      - g4s.kube.large
  workflows:
    - setup-monitoring      # Run first
    - deploy-apps          # Run second
    - configure-ingress    # Run third
```
...
<AccordionGroup>
  ...
  <Accordion title="3. Include Essential Workflows">
    Add workflows for consistent configuration:
...
### Production Template with Workflows
...
# [INFO] Running workflow 'setup-monitoring'
# [INFO] Workflow 'setup-monitoring' completed
# [INFO] Running workflow 'setup-logging'
