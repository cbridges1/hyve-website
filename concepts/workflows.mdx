...
When a workflow is assigned to a `beforeCreate` or `afterDelete` hook, the reconciler
automatically injects the following HYVE_* variables from the cluster definition before
the workflow runs — even though no live cluster exists at that point:

| Variable | Source |
|----------|--------|
| `HYVE_CLUSTER_NAME` | `metadata.name` |
| `HYVE_CLUSTER_REGION` | `metadata.region` |
| `HYVE_CLUSTER_PROVIDER` | `spec.provider` |
| `HYVE_CLUSTER_TYPE` | `spec.clusterType` |
| Provider credentials | From the matching `provider-configs/*.yaml` entry |

`onCreate` and `onDestroy` hooks additionally receive live cluster data
(`HYVE_CLUSTER_IP_ADDRESS`, `HYVE_CLUSTER_ID`, `HYVE_CLUSTER_STATUS`, `KUBECONFIG`).
