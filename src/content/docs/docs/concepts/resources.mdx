...
What Hyve applied is recorded in `spec.appliedResources`, keyed by resource name — reconciler-owned, never hand-edit, the same convention as `spec.driverOutputs`. Both live in a separate reconciler-owned file, `clusters/<name>.state.yaml`, alongside the cluster's desired-state YAML — see [Reconciler state file](/docs/concepts/clusters#reconciler-state-file). Inspect the tracked state with [`hyve cluster resources <name>`](/docs/cli/cluster#hyve-cluster-resources).
