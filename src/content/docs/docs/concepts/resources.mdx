...
spec:
  resources:
    ...
    - name: portainer-public-resource
      # The same manifest file can be reused across templates/clusters —
      # each reference supplies its own values here, so the file itself
      # never hardcodes anything cluster-specific.
      source: ./resource-files/portainer-public-resource.yaml
      values:
        FULL_DOMAIN: portainer.example.com
    ...
    - name: cert-manager
      helm:
        ...
        values:
          installCRDs: "true"
          # ${VAR} expands against Hyve's own process environment at
          # reconcile time — useful for a value that's real but not secret
          # (an org ID, an API endpoint) and shouldn't be committed
          # literally into a template reused across many cluster instances.
...
<ParamField path="values" type="object">
  `${VAR_NAME}` substitutions (braced form only) applied to this resource's manifest file — the raw text, comments included. Only meaningful with `source`. Lets one manifest file be reused across multiple resource entries — even within the same template — while each reference supplies its own values, instead of hardcoding something reference-specific (a hostname, an org ID) directly into a shared file.

  Three-tier lookup per reference, most specific wins:
  1. This resource's own `values:` — verbatim keys, no case transform, exactly like `helm.values`.
  2. The owning cluster's `spec.params`, as `HYVE_PARAM_<KEY>` (see [the `HYVE_PARAM_*` naming convention](/docs/concepts/modules#module-params)) — a template-wide default when every resource sharing a file wants the same value.
  3. Hyve's own process environment (`.env` locally, CI secrets) — same fallback `helm.values` already gets.

  A referenced variable that resolves in none of the three is a hard reconcile error naming it, not a silent empty-string substitution — same fail-loud behavior as `helm.values` and [Secret resources](#secret-resources).

  <Warning>
    This scans the resource's *entire* raw manifest text, not just specific fields — including anything inside a container's own `command`/`script`. Leaving `values:` unset skips this scan completely, so a manifest with its own unrelated `${...}` text (a shell script that legitimately sets and reads its own variable, say) is left untouched — but a manifest that *does* declare `values:` has its whole file scanned, so an incidental `${...}` elsewhere in that same file will be treated as a reference needing to resolve too. Confirmed the hard way: a `source:` resource with no `values:` at all broke when this feature first shipped without the empty-`values:` skip, because a container's own `${PRIVATE_KEY}` shell reference got caught by the same regex.
  </Warning>
</ParamField>
