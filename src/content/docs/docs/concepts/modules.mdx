...
<Info>
  Operations don't have to be written *in* shell — they just have to be invoked as a shell entry point. Hyve always runs the operation file (`create.yaml`, `create.sh`, or a bare `create`) through `/bin/sh`, but that file can be a one-line wrapper that calls out to a Python script, a compiled Go binary, Terraform, Pulumi, or anything else — the implementation language is entirely up to you. The only real requirement is the output contract: print `HYVE_KEY=value` lines (and `HYVE_CLUSTER_STATUS=<value>` for `status`) to stdout. See [Outputs protocol](#outputs-protocol).
</Info>
...
| `HYVE_CLUSTER_REGION` | `cluster.spec.region` |
...
Keep `hyve.lock` in your environment directory and share it however you share the rest of that directory (committed to git, if you're using it — see [Environments](/docs/concepts/repositories)). This ensures:
...
`cluster.spec.driverOutputs` is a key-value map populated by the reconciler after every successful create (or any operation that emits `HYVE_KEY=value` lines). It's written to a reconciler-owned sidecar file (`cluster-state/<name>.state.yaml`, a sibling of the `clusters/` directory the desired-state YAML lives in), not inline in it — see [Reconciler state file](/docs/concepts/clusters#reconciler-state-file). If your environment directory is git-backed, that sidecar file gets committed like any other change; hyve itself never commits it for you.
