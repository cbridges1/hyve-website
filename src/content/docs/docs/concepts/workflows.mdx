---
title: "Workflow Concepts"
description: "Understanding automated deployment pipelines and workflow execution"
---

import Card from '../../../../components/mdx/Card.astro';
import CardGroup from '../../../../components/mdx/CardGroup.astro';
import Steps from '../../../../components/mdx/Steps.astro';
import Step from '../../../../components/mdx/Step.astro';
import Accordion from '../../../../components/mdx/Accordion.astro';
import AccordionGroup from '../../../../components/mdx/AccordionGroup.astro';
import Note from '../../../../components/mdx/Note.astro';
import Warning from '../../../../components/mdx/Warning.astro';
import ParamField from '../../../../components/mdx/ParamField.astro';

## Overview

Workflows in Hyve are automated deployment pipelines defined as YAML files in Git repositories. They enable consistent, repeatable operations with built-in validation, secret management, and variable substitution.

<CardGroup cols={2}>
  <Card title="Automated Pipelines" icon="diagram-project" color="#22c55e">
    Define multi-step deployment processes
  </Card>

  <Card title="Requirements Validation" icon="list-check" color="#22c55e">
    Ensure tools and secrets are available before execution
  </Card>

  <Card title="Variable Substitution" icon="brackets-curly" color="#eab308">
    Use environment and workflow variables
  </Card>

  <Card title="Version Controlled" icon="code-branch" color="#8b5cf6">
    Workflows stored in Git with full history
  </Card>
</CardGroup>

## Workflow Structure

Workflows follow a standard YAML structure:

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: deploy-app
  description: Deploy application to Kubernetes
tools:
  - name: kubectl
    min: "1.28.0"
spec:
  requirements:
    secrets:
      - name: DOCKER_TOKEN
        provider: docker
  env:
    APP_NAME: my-app
    NAMESPACE: default
  jobs:
    - name: deploy
      steps:
        - name: apply-manifests
          command: kubectl apply -f manifests/
```

### Metadata

<ParamField path="metadata.name" type="string" required>
  Unique workflow identifier
</ParamField>

<ParamField path="metadata.description" type="string">
  Human-readable description of the workflow
</ParamField>

### Spec

<ParamField path="spec.inputs" type="array">
  Variables that must be present before the workflow runs. When the workflow is executed
  as a lifecycle hook (`beforeCreate`, `onCreate`, `onDelete`, `afterDelete`, `preReconcile`),
  values are injected automatically by the reconciler. When run ad-hoc via `hyve workflow run`,
  any missing inputs are prompted for in the TUI or must be supplied with `--set KEY=VALUE` on the CLI.
  See [Workflow Inputs](#workflow-inputs) for full details.
</ParamField>

<ParamField path="spec.preFlight.cluster" type="string">
  Set to `"skip"` to bypass the kubeconfig setup step before execution. Required for `beforeCreate`
  workflows that need to authenticate to a cloud provider before the cluster exists.
</ParamField>

<ParamField path="spec.requirements" type="object">
  Prerequisites for workflow execution (tools and secrets)
</ParamField>

<ParamField path="spec.env" type="object">
  Environment variables available to all jobs
</ParamField>

<ParamField path="spec.jobs" type="array" required>
  List of jobs to execute sequentially
</ParamField>

## Jobs and Steps

Jobs contain steps that execute commands:

```yaml
jobs:
  - name: build
    steps:
      - name: docker-build
        command: docker build -t ${APP_NAME}:latest .
      - name: docker-push
        command: docker push ${APP_NAME}:latest

  - name: deploy
    dependsOn: [build]
    steps:
      - name: kubectl-apply
        command: kubectl apply -f manifests/
```

### Job Fields

<ParamField path="jobs[].name" type="string" required>
  Job identifier
</ParamField>

<ParamField path="jobs[].dependsOn" type="array">
  List of job names that must complete before this job runs
</ParamField>

<ParamField path="jobs[].steps" type="array" required>
  List of steps to execute in sequence
</ParamField>

### Step Fields

<ParamField path="steps[].name" type="string" required>
  Step identifier
</ParamField>

<ParamField path="steps[].command" type="string">
  Single command to execute
</ParamField>

<ParamField path="steps[].script" type="string">
  Multi-line shell script to execute
</ParamField>

## Requirements

Workflows can validate prerequisites before execution:

### Tool Requirements

Declare required CLI tools under `spec.requirements.tools`. A version is optional — a bare
`name:` entry means any installed version is acceptable; `version:` (if set) is checked
against whatever `<tool> --version`/`-v`/`version` reports, matching or newer.

```yaml
spec:
  requirements:
    tools:
      - name: kubectl
        version: "1.28.0"
      - name: helm
        version: "3.12.0"
      - name: docker        # any version — just needs to be present
```

Checked with a plain `PATH` lookup (`exec.LookPath`) before the workflow runs — this is a
per-workflow, self-contained check. There's no `hyve.yaml`-level tools list it cross-references
and no coordination across workflows; each workflow's `requirements.tools` stands alone. There
is no separate hyve-level tools manifest today — whatever's running `hyve reconcile`/`hyve
workflow run` (a CI runner, your own machine, a container image you control) needs these tools
on its own `PATH` already.

### Secret Requirements

Load secrets from the environment:

```yaml
spec:
  requirements:
    secrets:
      - name: DOCKER_TOKEN
        required: true
        description: Docker Hub authentication
      - name: GITHUB_TOKEN
        required: false
        description: GitHub API token (optional)
```

Checked with `os.Getenv` — if a required secret's environment variable isn't set, the workflow fails before running with a clear error. `provider` is optional and only changes the *suggestion text* in that error for a handful of recognized values (`civo`, `aws`, `gcp`, `azure` — e.g. suggesting `aws configure`); it doesn't look anything up on its own.

<Note>
  See [Workflow Requirements](/docs/workflows/requirements) for detailed documentation.
</Note>

## Workflow Inputs

`spec.inputs` declares the environment variables a workflow needs at runtime. This is distinct from `spec.requirements.secrets` (which loads secrets from storage) — inputs are values the **caller** must supply.

```yaml
spec:
  inputs:
    - name: HYVE_CLUSTER_NAME
      description: "Name of the cluster to provision infrastructure for"
    - name: HYVE_CLUSTER_REGION
      description: "Cloud region to create resources in"
    - name: HYVE_CLUSTER_PROVIDER
      description: "Cloud provider (azure, aws, gcp, civo)"
  jobs:
    - name: provision-network
      steps:
        - name: create-vnet
          command: |
            az network vnet create \
              --name ${HYVE_CLUSTER_NAME}-vnet \
              --location ${HYVE_CLUSTER_REGION} \
              --resource-group ${HYVE_CLUSTER_NAME}-rg
```

### Automatic injection by lifecycle hooks

There's no fixed, hyve-defined set of provider-specific variables (no `HYVE_CLUSTER_PROVIDER`, `HYVE_AWS_ACCOUNT`, or similar) — injection is the same simple, three-part convention every module operation also gets:

| Variable | Source | Available in |
|----------|--------|--------------|
| `HYVE_CLUSTER_NAME` | `metadata.name` | All hooks |
| `HYVE_CLUSTER_REGION` | `spec.region` | All hooks |
| `HYVE_PARAM_<KEY>` | Each `spec.params` entry, key uppercased | All hooks |
| `<key>` (as-is) | Each `spec.driverOutputs` entry — whatever a prior module operation or workflow emitted, under whatever name it chose | All hooks |

<Note>
  `driverOutputs` keys are a module's own convention, not hyve's — a Civo-style module commonly emits `HYVE_CLUSTER_ID`/`HYVE_CLUSTER_STATUS`, but that's the module author's choice, not a guarantee every module provides. Check the specific module's own docs for what it actually sets.
</Note>

`onCreate`, `afterCreate`, and `onDelete` hooks additionally get `KUBECONFIG` pointed at the cluster's merged kubeconfig — `beforeCreate` and `afterDelete` run without a live cluster, so no kubeconfig is available.

A `beforeCreate` step's `HYVE_KEY=value` output lines are captured and merged into `spec.driverOutputs` before creation proceeds — so a later step (in the same or a later workflow, or the driver module's own `create.yaml`) can reference `<KEY>` as a plain env var, exactly like any other `driverOutputs` entry. There's no separate declaration needed for a specific value to "count" — whatever a `beforeCreate` step outputs becomes available this way automatically.

### Ad-hoc runs

When the same workflow is run outside a cluster lifecycle (e.g., to manually provision
infrastructure), missing inputs are supplied with `--set`:

```bash
hyve workflow run provision-network \
  --set HYVE_CLUSTER_NAME=my-cluster \
  --set HYVE_CLUSTER_REGION=eastus
```

<ParamField path="inputs[].name" type="string" required>
  Environment variable name (e.g. `HYVE_CLUSTER_NAME`)
</ParamField>

<ParamField path="inputs[].description" type="string">
  Human-readable label shown in the TUI prompt and in `hyve workflow show` output
</ParamField>

<ParamField path="inputs[].default" type="string">
  Default value used when no value is supplied. Shown as placeholder in the TUI prompt.
</ParamField>

## Environment Variables

Define variables at workflow or job level:

### Workflow-Level Variables

Available to all jobs:

```yaml
spec:
  env:
    APP_NAME: my-app
    REGISTRY: docker.io/myuser
    VERSION: v1.0.0
  jobs:
    - name: build
      steps:
        - name: build
          command: docker build -t ${REGISTRY}/${APP_NAME}:${VERSION} .
```

### Job-Level Variables

Available only to specific job:

```yaml
jobs:
  - name: deploy
    env:
      NAMESPACE: production
      REPLICAS: "3"
    steps:
      - name: deploy
        command: kubectl scale deployment ${APP_NAME} --replicas=${REPLICAS} -n ${NAMESPACE}
```

### Variable Substitution

Hyve supports full shell variable substitution:

```yaml
steps:
  - name: example
    command: echo "Deploying ${APP_NAME} version ${VERSION:-latest} to ${NAMESPACE}"
```

<Note>
  See [Variable Substitution](/docs/workflows/variables) for advanced usage.
</Note>

## Running Workflows

### Basic Execution

```bash
# Run workflow
hyve workflow run deploy-app

# Run with specific cluster
hyve workflow run deploy-app --cluster production
```

### Lifecycle Workflows

Workflows can be triggered automatically during cluster lifecycle events:

#### In Templates

```yaml
# Template definition
apiVersion: hyve.io/v1alpha1
kind: Template
metadata:
  name: prod-template
spec:
  # ... cluster config
  workflows:
    beforeCreate:
      - provision-vpc
    onCreate:
      - setup-monitoring
      - deploy-app
    afterCreate:
      - create-app-secrets
    onDelete:
      - backup-data
    afterDelete:
      - cleanup-vpc
```

#### In Cluster Definitions

```yaml
# Cluster definition
apiVersion: hyve.io/v1alpha1
kind: ClusterDefinition
metadata:
  name: production
spec:
  region: NYC1
  # ... cluster config
  workflows:
    beforeCreate:
      - provision-network
    onCreate:
      - setup-monitoring
    afterCreate:
      - create-app-secrets
    onDelete:
      - backup-data
    afterDelete:
      - cleanup-network
```

| Hook | When It Runs | Cluster Exists? | Kubeconfig Available? | Typical Use Cases |
|------|--------------|-----------------|----------------------|-------------------|
| `beforeCreate` | Before cluster is created | No | No | Provision VPC, create IAM roles, allocate IP ranges |
| `onCreate` | After cluster is ready, before `spec.resources` applies | Yes | Yes | Setup monitoring, deploy apps, configure services |
| `afterCreate` | After cluster is ready, after `spec.resources` has applied | Yes | Yes | Create a Secret a resource-managed Deployment references, DNS pointing at a now-deployed app |
| `onDelete` | Before cluster is deleted | Yes | Yes | Backup data, export logs, cleanup external resources |
| `afterDelete` | After cluster is deleted | No | No | Destroy VPC, remove IAM roles, release IP ranges |
| `preReconcile` | When param drift is detected on an ACTIVE cluster | Yes | Yes | Refresh credentials, sync config before scaling |

<Note>
`beforeCreate` and `afterDelete` run when no cluster exists, so kubeconfig injection is skipped. Workflows assigned to these hooks must not use `kubectl` or any tool that requires cluster access.

`preReconcile` runs immediately before the module's scale operation, only when the reconciler detects `spec.params` has changed since the last successful reconcile — it does **not** run on every reconcile cycle. If you need something to run unconditionally on every cycle for an ACTIVE cluster, that's what [`spec.resources`](/docs/concepts/resources) is for — it drift-checks and re-applies regardless of param drift, every time.
</Note>

### Remote Workflow References

Every entry above can also be a remote reference instead of a local name — a workflow pulled from another Git repository, resolved and content-hashed into `hyve.lock`, the same way modules are referenced via `spec.driver.source`:

```yaml
spec:
  workflows:
    onCreate:
      - pre-flight-checks                                                  # local, unchanged
      - source: github.com/myorg/shared-workflows//setup-monitoring.yaml@v1.2.0   # remote, pinned
      - source: github.com/myorg/shared-workflows//setup-monitoring.yaml          # remote, tracks latest
```

This exists so shared operational workflows — monitoring setup, backup jobs, teardown routines — can live in one repository and be referenced from many, instead of being copied into every state repository that needs them. Version is optional: omit it and Hyve resolves the latest commit once and locks the resolved SHA, exactly like an unversioned module reference.

A lifecycle hook entry must resolve to a single workflow file — a directory-form source (used for bulk-installing a whole set of workflows via `hyve workflow install`) is rejected if placed directly in a hook. See [`hyve workflow`](/docs/cli/workflow#remote-workflow-references) for the full source-string format, path resolution rules, and name-resolution order for `hyve workflow run`.

### Auth-Bootstrap Workflows (preFlight.cluster: skip)

Some `beforeCreate` workflows need to authenticate to a cloud provider to provision
resources (e.g. create an EKS IAM role or VPC) before the cluster exists. By default
Hyve tries to set up kubeconfig before running any workflow. Set `preFlight.cluster: skip`
in the workflow spec to bypass the kubeconfig setup step:

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: provision-iam-roles
spec:
  preFlight:
    cluster: skip    # skip EKS DescribeCluster + kubeconfig sync
  jobs:
    - name: create-roles
      steps:
        - name: create-eks-role
          script: |
            aws iam create-role --role-name eks-cluster-role \
              --assume-role-policy-document file://eks-trust-policy.json
            echo "HYVE_EKS_ROLE_NAME=eks-cluster-role"
```

The `HYVE_EKS_ROLE_NAME=eks-cluster-role` output line is captured by the reconciler as a
driver output and injected as the `HYVE_EKS_ROLE_NAME` environment variable into every
later hook and module operation for this cluster — for example the module's `create`
operation can read it to reference the role it depends on.

### Execution Flow

<Steps>
  <Step title="Load Workflow">
    Read workflow YAML from repository

    ```bash
    # workflows/deploy-app.yaml
    ```
  </Step>

  <Step title="Validate Requirements">
    Check tools and secrets

    ```
    [INFO] Validating workflow requirements...
    [INFO] ✅ All requirements validated successfully
    ```
  </Step>

  <Step title="Set Environment">
    Load environment variables and secrets

    ```bash
    export APP_NAME=my-app
    export DOCKER_TOKEN=***
    ```
  </Step>

  <Step title="Execute Jobs">
    Run jobs in sequence

    ```
    [INFO][build] Starting job 'build'
    [INFO][build][docker-build] Running step 'docker-build'
    ```
  </Step>

  <Step title="Report Results">
    Show success or failure

    ```
    [INFO] Workflow 'deploy-app' completed successfully
    ```
  </Step>
</Steps>

## Workflow Types

### Deployment Workflow

Deploy applications to Kubernetes:

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: deploy-app
tools:
  - name: kubectl
    min: "1.28.0"
spec:
  env:
    APP_NAME: my-app
  jobs:
    - name: deploy
      steps:
        - name: apply
          command: kubectl apply -f manifests/
        - name: wait
          command: kubectl rollout status deployment/${APP_NAME}
```

### Build Workflow

Build and push Docker images:

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: build-image
tools:
  - name: docker
spec:
  requirements:
    secrets:
      - name: DOCKER_TOKEN
        provider: docker
  env:
    IMAGE: myapp
    TAG: latest
  jobs:
    - name: build
      steps:
        - name: docker-build
          script: |
            echo "$DOCKER_TOKEN" | docker login -u myuser --password-stdin
            docker build -t ${IMAGE}:${TAG} .
            docker push ${IMAGE}:${TAG}
```

### Infrastructure Workflow

Provision infrastructure with Terraform:

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: provision-infra
tools:
  - name: terraform
    min: "1.5.0"
spec:
  requirements:
    secrets:
      - name: AWS_ACCESS_KEY_ID
        provider: aws
      - name: AWS_SECRET_ACCESS_KEY
        provider: aws
  jobs:
    - name: provision
      steps:
        - name: init
          command: terraform init
        - name: plan
          command: terraform plan
        - name: apply
          command: terraform apply -auto-approve
```

## Commands vs Scripts

### Single Commands

For simple operations:

```yaml
steps:
  - name: deploy
    command: kubectl apply -f manifests/
```

### Multi-Line Scripts

For complex operations:

```yaml
steps:
  - name: deploy
    script: |
      echo "Starting deployment..."
      kubectl apply -f manifests/
      kubectl rollout status deployment/my-app
      echo "Deployment complete!"
```

## Job Dependencies

Control execution order with dependencies:

```yaml
jobs:
  - name: test
    steps:
      - name: run-tests
        command: npm test

  - name: build
    dependsOn: [test]
    steps:
      - name: docker-build
        command: docker build -t myapp .

  - name: deploy
    dependsOn: [build]
    steps:
      - name: kubectl-apply
        command: kubectl apply -f manifests/
```

Execution order: `test` → `build` → `deploy`

## Error Handling

Workflows stop on first error:

```yaml
jobs:
  - name: deploy
    steps:
      - name: test
        command: npm test  # If this fails, workflow stops

      - name: deploy
        command: kubectl apply -f manifests/  # This won't run
```

<Warning>
  If any step fails, the workflow stops immediately. Design workflows with this in mind.
</Warning>

## Best Practices

<AccordionGroup>
  <Accordion title="1. Use Descriptive Names">
    Name workflows, jobs, and steps clearly:

    ```yaml
    metadata:
      name: deploy-api-production
      description: Deploy API service to production cluster

    jobs:
      - name: run-tests
        steps:
          - name: unit-tests
          - name: integration-tests
    ```
  </Accordion>

  <Accordion title="2. Declare Tool Requirements">
    Declare required tools at the top level with optional version constraints:

    ```yaml
    tools:
      - name: kubectl
        min: "1.28.0"
    spec:
      requirements:
        secrets:
          - name: DOCKER_TOKEN
            provider: docker
            required: true
    ```
  </Accordion>

  <Accordion title="3. Use Environment Variables">
    Avoid hardcoding values:

    ```yaml
    # Good
    env:
      NAMESPACE: production
    steps:
      - name: deploy
        command: kubectl apply -f manifests/ -n ${NAMESPACE}

    # Bad
    steps:
      - name: deploy
        command: kubectl apply -f manifests/ -n production
    ```
  </Accordion>

  <Accordion title="4. Document Workflows">
    Add descriptions explaining purpose:

    ```yaml
    metadata:
      name: deploy-app
      description: |
        Deploy application to Kubernetes cluster
        - Runs tests first
        - Builds Docker image
        - Deploys to cluster
        - Waits for rollout
    ```
  </Accordion>

  <Accordion title="5. Test in Development">
    Test workflows in development before production:

    ```bash
    # Switch to development
    hyve env use development

    # Test workflow
    hyve workflow run deploy-app --cluster dev-cluster

    # If successful, run in production
    hyve env use production
    hyve workflow run deploy-app --cluster prod-cluster
    ```
  </Accordion>
</AccordionGroup>

## Workflow Examples

### Complete Deployment Pipeline

```yaml
apiVersion: v1
kind: Workflow
metadata:
  name: complete-pipeline
  description: Full CI/CD pipeline
tools:
  - name: docker
  - name: kubectl
    min: "1.28.0"
  - name: helm
    min: "3.12.0"
spec:
  requirements:
    secrets:
      - name: DOCKER_TOKEN
        provider: docker
      - name: DATADOG_API_KEY
        provider: datadog
        required: false

  env:
    APP_NAME: my-app
    REGISTRY: docker.io/myuser
    VERSION: v1.0.0

  jobs:
    - name: test
      steps:
        - name: unit-tests
          command: npm test
        - name: lint
          command: npm run lint

    - name: build
      dependsOn: [test]
      steps:
        - name: docker-build
          script: |
            echo "$DOCKER_TOKEN" | docker login -u myuser --password-stdin
            docker build -t ${REGISTRY}/${APP_NAME}:${VERSION} .
            docker push ${REGISTRY}/${APP_NAME}:${VERSION}

    - name: deploy
      dependsOn: [build]
      env:
        NAMESPACE: production
      steps:
        - name: helm-upgrade
          command: |
            helm upgrade --install ${APP_NAME} ./chart \
              --set image.tag=${VERSION} \
              --namespace ${NAMESPACE}
        - name: wait-rollout
          command: kubectl rollout status deployment/${APP_NAME} -n ${NAMESPACE}

    - name: notify
      dependsOn: [deploy]
      steps:
        - name: send-notification
          script: |
            if [ -n "$DATADOG_API_KEY" ]; then
              echo "Sending notification to Datadog..."
              # Send notification
            fi
```

## Related

<CardGroup cols={2}>
  <Card title="Workflow Overview" icon="diagram-project" href="/docs/workflows/overview" color="#22c55e">
    Complete workflow guide
  </Card>

  <Card title="Requirements" icon="list-check" href="/docs/workflows/requirements" color="#22c55e">
    Tool and secret validation
  </Card>

  <Card title="Variables" icon="brackets-curly" href="/docs/workflows/variables" color="#eab308">
    Variable substitution
  </Card>

  <Card title="CLI Workflow" icon="terminal" href="/docs/cli/workflow" color="#10b981">
    Workflow command reference
  </Card>
</CardGroup>
