...
### 3. Reconciliation, Triggered Explicitly

Changes committed to Git are applied the next time you run `hyve reconcile` — reconciliation itself is automatic (it figures out what changed and converges infrastructure to match), but *triggering* a reconcile run, and keeping your local checkout in sync with the remote, are explicit steps you (or your CI/CD pipeline) control:
...
<Warning>
  Hyve does not pull, commit, or push git changes on your behalf — not before `hyve reconcile` runs, and not after it writes state back (e.g. `driverOutputs`, applied-resource tracking). See [Keeping a Directory in Sync](#keeping-a-directory-in-sync) below.
</Warning>

## Keeping a Directory in Sync

Hyve's core only ever reads and writes local files — it has no built-in git sync of any kind. If you're working with a git-backed directory (via `hyve git add`), you're responsible for keeping it in sync yourself:

```bash
# Before reconciling, make sure you have the latest state
hyve git pull
# (or: cd into the repo and run `git pull` directly)

hyve reconcile

# After reconciling, hyve may have written driverOutputs / applied-resource
# tracking back to cluster-state/*.state.yaml — push those changes yourself
hyve git push
# (or: git add -A && git commit -m "hyve: reconcile" && git push)
```

<Note>
  This is a deliberate design choice, not a missing feature — see [Repositories → Synchronization](/docs/concepts/repositories#synchronization) for the full reasoning. If you'd rather not think about a git remote at all, [`hyve set-state`](/docs/cli/state) registers a plain local directory with no git involvement whatsoever.
</Note>

In CI/CD, this pattern is exactly what you already want: `git pull && hyve reconcile && git commit -am "..." && git push` as pipeline steps, with your CI system's own retry/notification handling doing double duty as your sync-failure alerting.
...
<CardGroup cols={2}>
  <Card title="Repositories" icon="git" href="/docs/concepts/repositories" color="#8b5cf6">
  ...
  <Card title="Reconciliation" icon="arrows-rotate" href="/docs/guides/cluster-management#reconciliation" color="#22c55e">
