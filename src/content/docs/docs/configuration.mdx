...
Set it from the CLI — the filename is automatically appended to `.gitignore` the moment it's set, so it can't ship to your remote by omission:
...
See [`hyve config`](/docs/cli/config) for the full CLI reference.
...
<AccordionGroup>
  ...
  <Accordion title="Never commit secrets">
    ...
    If you set `env.file` in `hyve.yaml` (via `hyve config set env.file <path>` or Hyve
    Studio — see [`env` (optional)](#env-optional)), this is handled for you automatically:
