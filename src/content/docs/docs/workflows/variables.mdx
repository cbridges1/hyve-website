...
There's no fixed, hyve-defined set of provider-specific variables — injection is the same simple convention every module operation gets:

| Variable | Value | Notes |
|----------|-------|-------|
| `HYVE_CLUSTER_NAME` | Cluster name | |
| `HYVE_CLUSTER_REGION` | `spec.region` | |
| `HYVE_PARAM_<KEY>` | Each `spec.params` entry, key uppercased | One per param — entirely module-specific, no fixed list |
| `<key>` (as-is) | Each `spec.driverOutputs` entry | Whatever a prior module operation or `beforeCreate` step emitted, under whatever name it chose — a module's own convention, not hyve's |

### Live cluster variables (`onCreate`, `afterCreate`, `onDelete`)

| Variable | Value |
|----------|-------|
| `KUBECONFIG` | Path to the cluster's merged kubeconfig |

`beforeCreate` and `afterDelete` run without a live cluster — no kubeconfig is available at those points. Anything resembling `HYVE_CLUSTER_STATUS`/`HYVE_CLUSTER_ID` is a `driverOutputs` entry a specific module chose to emit (a common convention, e.g. in the Civo module), not a value hyve itself guarantees or computes.

### beforeCreate output variables

A `beforeCreate` step's `HYVE_KEY=value` output lines are captured and merged into `spec.driverOutputs` before creation proceeds — there's no fixed list of "supported" output variable names or a mapping to specific spec fields. Whatever key a step outputs becomes available as that same env var name to every later step/operation for this cluster (see the definition-variables row above).
