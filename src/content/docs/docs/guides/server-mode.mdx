...
| `PATCH` | `/config` | Partial update; writes through to `hyve.yaml` and commits. Changes to `server.*` only take effect on the next `hyve serve` restart. Setting `env.file` also updates `.gitignore` — see [Configuration](/docs/configuration#env-optional). |
...
<CardGroup cols={2}>
  <Card title="hyve serve CLI Reference" icon="terminal" href="/docs/cli/serve" color="#ec4899">
    Flags and usage for `hyve serve`, including `--open`
  </Card>
