...
<Info>
  hyve has no user or permission model of its own (see [What This Is Not](#what-this-is-not)), so "admin-only" isn't anything hyve enforces — it means not exposing this workflow's trigger to callers who shouldn't have it, at whatever layer sits in front of hyve (a UI that only shows it to admins, an API gateway rule, or simply not documenting it to non-admins). Treat it the same as any other privileged operation you'd gate before it ever reaches hyve.
</Info>
