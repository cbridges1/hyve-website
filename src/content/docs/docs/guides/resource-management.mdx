...
`spec.appliedResources` is now populated and committed (to `clusters/production.state.yaml`, not `production.yaml` itself — see [Reconciler state file](/docs/concepts/clusters#reconciler-state-file)) — inspect it any time with:
