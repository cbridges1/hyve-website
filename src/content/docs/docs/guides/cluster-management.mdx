---
title: "Cluster Management"
description: "Complete guide to creating, managing, and deleting Kubernetes clusters with Hyve"
---

import Card from '../../../../components/mdx/Card.astro';
import CardGroup from '../../../../components/mdx/CardGroup.astro';
import Accordion from '../../../../components/mdx/Accordion.astro';
import AccordionGroup from '../../../../components/mdx/AccordionGroup.astro';
import Warning from '../../../../components/mdx/Warning.astro';
import Note from '../../../../components/mdx/Note.astro';

## Overview

Hyve manages Kubernetes clusters through declarative YAML definitions in a local directory — [environments](/docs/concepts/repositories), git-backed or not. This guide covers the full lifecycle: installing modules, creating templates, provisioning clusters, managing them day-to-day, and safely deleting them.

<CardGroup cols={2}>
  <Card title="Declarative" icon="code-branch" color="#3b82f6">
    Cluster state is plain YAML — add git for full history, PR reviews, and rollbacks, or skip it
  </Card>

  <Card title="Module-based" icon="cube" color="#8b5cf6">
    Any cloud provider via versioned modules — no embedded SDKs
  </Card>

  <Card title="Lifecycle Hooks" icon="gears" color="#22c55e">
    beforeCreate, onCreate, afterCreate, onDelete, afterDelete hooks run automatically
  </Card>

  <Card title="Reproducible" icon="lock" color="#f97316">
    hyve.lock pins module versions for consistent reconciles across environments
  </Card>
</CardGroup>

## Prerequisites

Before managing clusters you need:

1. Hyve installed (`go install github.com/cbridges1/hyve@latest`)
2. An environment registered (`hyve env create production --path ~/repos/hyve-prod`)
3. The cloud CLI for your provider installed and authenticated

See [Installation](/docs/installation) and [Quick Start](/docs/quickstart) for setup.

## Step 1 — Install a module

Modules implement cluster operations for a specific provider — Hyve has no built-in notion of any particular cloud. Install one and lock it in `hyve.lock`:

```bash
# Ensure the provider's own CLI is authenticated first, e.g.:
civo apikey save my-token YOUR_TOKEN && civo apikey use my-token

hyve module add github.com/hyve-modules/civo@v1.0.0
```

<Note>
  Any module works the same way — swap the source for EKS, GKE, AKS, or your own. See [First-party modules](/docs/cli/module#first-party-modules) for the list.
</Note>

Verify the module is locked:

```bash
hyve module list
```

## Step 2 — Create a template

Templates define the cluster shape, driver, region, and default params. Create one for each cluster pattern in your infrastructure:

```bash
# Development cluster
hyve template create dev-civo \
  --driver github.com/hyve-modules/civo \
  --driver-version v1.0.0 \
  --region PHX1 \
  --set node_size=g4s.kube.small \
  --set node_count=1

# Production cluster
hyve template create prod-civo \
  --driver github.com/hyve-modules/civo \
  --driver-version v1.0.0 \
  --region NYC1 \
  --set node_size=g4s.kube.large \
  --set node_count=3
```

<Note>
  `hyve template create` only sets `driver`/`region`/`params`. To add `onCreate`/`onDelete` (or any other) lifecycle workflows, edit the generated YAML afterward — there's no per-hook creation flag. See [Template Management](/docs/guides/template-management#template-structure).
</Note>

## Step 3 — Create a cluster

Execute a template to generate the cluster YAML and provision it:

```bash
hyve cluster create my-prod-cluster --template prod-civo
```

**Override template defaults at cluster-creation time:**

```bash
# Override region and node count for this specific cluster
hyve cluster create eu-cluster --template prod-civo \
  --region FRA1 \
  --set node_count=5
```

## Step 4 — Access the cluster

After the cluster is provisioned, run the auth operation to merge the kubeconfig:

```bash
hyve cluster auth my-prod-cluster
kubectl config use-context my-prod-cluster
kubectl get nodes
```

## Listing and inspecting clusters

```bash
# List all clusters in the active repository
hyve cluster list

# Show full YAML and summary for a specific cluster
hyve cluster show my-prod-cluster
```

## Multi-environment strategy

Manage separate directories for different environments — see [Environments](/docs/concepts/repositories):

```bash
# Register each environment
hyve env create production --path ~/repos/hyve-prod
hyve env create staging --path ~/repos/hyve-staging
hyve env create development --path ~/repos/hyve-dev

# Switch active environment
hyve env use staging

# Create cluster in staging
hyve cluster create staging-cluster --template prod-civo --set node_count=2

# Switch to production
hyve env use production
hyve cluster create prod-cluster-01 --template prod-civo
```

## Scaling clusters

To change a cluster's scale, edit the cluster YAML directly:

```bash
# Edit the cluster YAML
vim <environment directory>/clusters/my-cluster.yaml
# Change: node_count: "3" → node_count: "5"

# If your environment directory is git-backed, commit it (optional)
cd <environment directory>
git add clusters/my-cluster.yaml
git commit -m "Scale my-cluster to 5 nodes"
git push

# Reconcile to apply
hyve reconcile
```

Or update the template and re-execute for new clusters:

```bash
# Update the template default
hyve template create prod-civo-large \
  --driver github.com/hyve-modules/civo \
  --driver-version v1.0.0 \
  --region NYC1 \
  --set node_size=g4s.kube.large \
  --set node_count=5

# Create a new cluster from the updated template
hyve cluster create my-new-cluster --template prod-civo-large
```

## Pausing clusters

Pause a cluster to temporarily prevent Hyve from touching it (useful during maintenance):

```bash
# Set spec.pause: true in the cluster YAML
vim <environment directory>/clusters/my-cluster.yaml
# Change: pause: false → pause: true

# If your environment directory is git-backed, commit it (optional)
git add clusters/my-cluster.yaml
git commit -m "Pause my-cluster for maintenance"
git push
```

When paused, the reconciler skips the cluster entirely. To resume, set `pause: false`.

## Setting cluster expiry

For ephemeral clusters, set an auto-delete timestamp:

```bash
# Set expiry in the cluster YAML
vim <environment directory>/clusters/dev-cluster.yaml
# Set: expiresAt: "2026-09-01T00:00:00Z"

# If your environment directory is git-backed, commit it (optional)
git add clusters/dev-cluster.yaml
git commit -m "Set dev-cluster to expire on 2026-09-01"
git push
```

Or use a template with a `--schedule` to auto-compute the expiry:

```bash
hyve template create dev-weekly \
  --driver github.com/hyve-modules/civo \
  --driver-version v1.0.0 \
  --region PHX1 \
  --set node_size=g4s.kube.small \
  --schedule "0 20 * * 5"   # auto-expires every Friday at 20:00

hyve cluster create my-dev-cluster --template dev-weekly
```

## Deleting clusters

To delete a cluster and ensure lifecycle hooks run:

```bash
hyve cluster delete my-cluster
```

This sets `spec.delete: true` in the cluster YAML, commits, and reconciles. The reconciler runs `onDelete` workflows, calls the module's delete operation, runs `afterDelete` workflows, and removes the YAML.

<Warning>
  Do not delete the cluster YAML file directly — lifecycle hooks (`onDelete`, `afterDelete`) will not run. Always use `hyve cluster delete`.
</Warning>

## Using lifecycle hooks

Lifecycle hooks let you run arbitrary workflows at each stage of a cluster's life. The hooks are defined in the cluster YAML under `spec.workflows`:

```yaml
spec:
  workflows:
    beforeCreate:
      - provision-network     # Create VPC before the cluster is provisioned
    onCreate:
      - setup-monitoring      # Install monitoring stack after cluster is ready
      - deploy-app            # Deploy application
    onDelete:
      - backup-data           # Backup before deletion
      - drain-workloads       # Drain nodes gracefully
    afterDelete:
      - cleanup-network       # Remove VPC after cluster is gone
```

See the [Workflow Management guide](/docs/guides/workflow-management) for how to define workflow YAMLs.

## Blue-green deployments

Deploy a new cluster alongside the existing one, test, then delete the old one:

```bash
# Create green cluster
hyve cluster create prod-green --template prod-civo \
  --set node_size=g4s.kube.large \
  --set node_count=3

# Test green cluster
hyve cluster auth prod-green
kubectl config use-context prod-green
kubectl get pods

# Switch traffic (update DNS/load balancer externally)
# ...

# Delete blue cluster when green is stable
hyve cluster delete prod-blue
```

## Reconciliation modes

Control reconciliation behaviour with `hyve.yaml` at your repo root:

```yaml
reconcile:
  mode: local          # or "cicd"
  strictDelete: false  # set to true to warn about unmanaged clusters (see /docs/guides/cicd)
```

| Mode | When to use |
|------|-------------|
| `local` | Local development — Hyve provisions directly from your machine |
| `cicd` | CI/CD pipelines — Hyve validates and pushes to Git; pipeline runs `hyve reconcile --path .` |

See the [CI/CD guide](/docs/guides/cicd) for a full GitHub Actions example.

## Troubleshooting

<AccordionGroup>
  <Accordion title="Module not locked">
    Error: `Module github.com/... is not locked in hyve.lock`

    ```bash
    hyve module install     # lock all template modules
    # or
    hyve module add github.com/hyve-modules/civo@v1.0.0
    ```
  </Accordion>

  <Accordion title="Credentials not found">
    The module cannot find cloud credentials. Ensure the cloud CLI is authenticated:

    ```bash
    # Civo
    civo apikey list

    # AWS
    aws sts get-caller-identity

    # GCP
    gcloud auth application-default print-access-token

    # Azure
    az account show
    ```
  </Accordion>

  <Accordion title="Cluster not appearing after execution">
    Run reconciliation manually to trigger provisioning:

    ```bash
    hyve reconcile
    hyve cluster list
    ```
  </Accordion>

  <Accordion title="Cannot access cluster after creation">
    Run the auth operation to merge the kubeconfig:

    ```bash
    hyve cluster auth my-cluster
    kubectl config use-context my-cluster
    kubectl cluster-info
    ```
  </Accordion>
</AccordionGroup>

## Related

<CardGroup cols={2}>
  <Card title="Module System" icon="cube" href="/docs/concepts/modules" color="#8b5cf6">
    How modules work and how to write your own
  </Card>

  <Card title="Workflow Management" icon="diagram-project" href="/docs/guides/workflow-management" color="#22c55e">
    Define lifecycle workflows
  </Card>

  <Card title="Template Management" icon="layer-group" href="/docs/guides/template-management" color="#06b6d4">
    Create and manage reusable templates
  </Card>

  <Card title="CLI: Cluster" icon="terminal" href="/docs/cli/cluster" color="#10b981">
    Complete cluster command reference
  </Card>
</CardGroup>
