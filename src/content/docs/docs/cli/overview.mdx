...
<CardGroup cols={3}>
  ...
  <Card title="config" icon="gear" href="/docs/cli/config" color="#64748b">
...
```bash
hyve set-state [path] [--current]
hyve state
```

Registers a plain local directory as the active state directory — no git repository or remote required. A lighter-weight alternative to `hyve git add` for local development.

[Full reference →](/docs/cli/state)
...
Reads and updates `hyve.yaml`.

[Full reference →](/docs/cli/config)

<Info>
  `hyve serve` and `hyve tui` have been removed — see [hyve serve](/docs/cli/serve) and [hyve tui](/docs/cli/tui) for what changed.
</Info>
