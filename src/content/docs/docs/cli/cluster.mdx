...
<Info>
  This reads `spec.resources`/`spec.appliedResources` directly from the cluster YAML — it does not run `kubectl diff` or contact the live cluster, so it works even offline. Use `hyve reconcile --dry-run` for a live drift preview. See [Cluster Resources](/docs/concepts/resources).
</Info>
  ...
  resources:
    ...
    - name: cert-manager
      helm:
        ...
        values:
          installCRDs: "true"
  appliedResources: {}          # populated by reconciler — do not edit manually
...
<ParamField path="spec.resources" type="array">
  Kubernetes manifests or Helm releases Hyve should own, drift-check, and re-apply on every reconcile cycle for an ACTIVE cluster — unconditionally, not gated by param drift. See [Cluster Resources](/docs/concepts/resources) for the full schema.
</ParamField>

<ParamField path="spec.appliedResources" type="object">
  Populated by the reconciler after successfully applying a resource. Do not edit manually — mirrors `driverOutputs` at resource granularity. Use `hyve cluster resources <name>` to inspect it.
</ParamField>
