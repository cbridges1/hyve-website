...
The `hyve cluster` command group manages Kubernetes cluster definitions stored in your active [environment](/docs/concepts/repositories), and provides access to cluster authentication.

## Overview

Clusters are defined as YAML files in the `clusters/` directory of your active environment — a local directory, git-backed or not (see [Version Control](/docs/concepts/gitops)), or a live cluster-mode API. They specify a driver (module), region, params, and optional lifecycle workflows. The `hyve cluster` commands let you create, list, inspect, mark for deletion, and authenticate into clusters.
...
Create a cluster from a template. Generates the cluster YAML and runs reconciliation. If your directory is git-backed, committing and pushing that new file is your own step — see [Version Control](/docs/concepts/gitops).
...
5. Run reconciliation (create cluster via module, then run lifecycle workflows)
...
This sets `spec.delete: true` in the cluster YAML and (in local mode) immediately runs reconciliation. The reconciler will:
...
```
Marked cluster 'my-cluster' for deletion (spec.delete=true in clusters/my-cluster.yaml).
Running reconciliation...
...
<Warning>
  In CI/CD mode (`reconcile.mode: cicd`), setting `spec.delete: true` and running `hyve reconcile` validates and exits — commit and push the change yourself. The actual deletion happens when your CI/CD pipeline runs `hyve reconcile --path .`.
</Warning>
...
```bash
hyve cluster auth <name> [--method <method-name>] [--set KEY=VALUE ...]
```
...
<ParamField path="--set" type="string (repeatable)">
  `KEY=VALUE` — an explicit credential value for a cluster using an [Access Method](/docs/concepts/access-method), instead of expecting it already set in your shell's environment. Takes precedence over the same name found in your environment; useful for scripting/CI. Has no effect for a cluster using ordinary module auth.
</ParamField>
...
# A cluster whose access is governed by an Access Method (e.g. Rancher) —
# pass its required credentials explicitly instead of exporting them first
hyve cluster auth prod-workload --set RANCHER_USERNAME=alice --set RANCHER_PASSWORD=********
```

<Info>
  If the cluster's `ClusterDefinition` references an [Access Method](/docs/concepts/access-method) (`spec.access.accessMethodRef`), this requires an active `hyve login` session — the access method's auth logic always runs server-side, in a short-lived Job, never on your machine.
</Info>
...
`driverOutputs` and `appliedResources` are deliberately not shown here — they're reconciler-owned and live in a separate sidecar file, `cluster-state/my-cluster.state.yaml`, never inline in this one. See [Reconciler state file](/docs/concepts/clusters#reconciler-state-file).
...
<ParamField path="spec.driverOutputs" type="object">
  Populated by the reconciler after successful create/update operations. Do not edit manually. All keys are re-injected verbatim as environment variables on the next reconcile. Physically stored in `cluster-state/<name>.state.yaml`, not this file — see [Reconciler state file](/docs/concepts/clusters#reconciler-state-file).
</ParamField>
...
- [hyve env](/docs/cli/env) — Register the directory or cluster target `hyve cluster` operates against
