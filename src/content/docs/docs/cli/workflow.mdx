...
📁 Location: <environment directory>/workflows/deploy-app.yaml
...
<Info>
  Works in both modes. In local mode, the workflow's steps run directly on your machine. Logged in via `hyve login` (cluster mode), this instead dispatches a `WorkflowRun` object that the controller executes the same way it runs a cluster's own lifecycle hooks (`afterCreate`, etc.) — the CLI polls its status and prints the result once it completes.
</Info>
...
- [hyve cluster](/docs/cli/cluster) - Manage clusters where workflows run, including `auth`/`deauth` for cluster access
