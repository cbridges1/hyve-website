...
Hyve reads and writes cluster state from a local directory — `hyve git add` registers one that's git-backed (with a remote URL), letting you use these commands to keep it in sync. You can configure multiple repositories for different environments (e.g., production, staging, development) and switch between them. If you don't need a git remote at all, [`hyve set-state`](/docs/cli/state) registers a plain local directory instead.

<Info>
  Hyve does not sync with git automatically. `hyve git pull`/`hyve git push`/`hyve git sync` below are explicit commands you (or your CI/CD pipeline) run — see [GitOps Principles → Keeping a Directory in Sync](/docs/concepts/gitops#keeping-a-directory-in-sync).
</Info>
...
<ParamField path="--keep-files" type="boolean" default="false">
...
Pass `--keep-files` to drop the configuration without touching the clone on disk — useful if `--path` pointed the repository at a directory you manage yourself.
