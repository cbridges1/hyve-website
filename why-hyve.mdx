...
In Hyve, `onCreated` and `onDestroy` workflow hooks run automatically as part of the same reconcile cycle. Define the steps once in a workflow YAML file; they execute every time a cluster is created or destroyed with no additional wiring.
...
The resulting cluster YAML is committed to the state repository, reconciled, and the `onCreated` workflows run — all in one step.
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in post-deploy hooks" icon="gears" color="#22c55e">
    onCreated and onDestroy workflows run automatically — no extra orchestration.
  </Card>
